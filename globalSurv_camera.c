...
const int EXTERNAL_PORT = 8080;           // Port to use for external access (configure in router)
String currentPublicIP = "";              // Will store current public IP
unsigned long lastPublicIPCheck = 0;
const unsigned long PUBLIC_IP_CHECK_INTERVAL = 300000; // Check every 5 minutes

// --- Security Configuration ---
const char* ACCESS_USERNAME = "admin";     // Change this!
const char* ACCESS_PASSWORD = "esp32cam";  // Change this!
bool authenticationEnabled = true;         // Set to false to disable authentication
...
bool authenticateUser(AsyncWebServerRequest *request) {
  ...
  if (!request->authenticate(ACCESS_USERNAME, ACCESS_PASSWORD)) {
    return false;
  }
  return true;
}
...
void updatePublicIP() {
  if (millis() - lastPublicIPCheck > PUBLIC_IP_CHECK_INTERVAL) {
    String newIP = getPublicIP();
    if (newIP != "" && newIP != currentPublicIP) {
      currentPublicIP = newIP;
      Serial.printf("Public IP updated: %s\n", currentPublicIP.c_str());
      Serial.printf("External access URL: http://%s:%d\n", currentPublicIP.c_str(), EXTERNAL_PORT);
    }
    lastPublicIPCheck = millis();
  }
}

// --- Web Interface with Internet Access Info ---
const char index_html[] PROGMEM = R"rawliteral(
<!DOCTYPE HTML>
<html>
<head>
  <title>ESP32-CAM Internet Controller</title>
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <style>
    body { font-family: Arial, sans-serif; margin: 0; padding: 20px; background: #1a1a1a; color: white; }
    .container { max-width: 800px; margin: 0 auto; }
    h1 { text-align: center; color: #4CAF50; }
    .access-info { background: #2a2a2a; padding: 15px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #4CAF50; }
    .access-info h3 { margin-top: 0; color: #4CAF50; }
    .access-url { background: #333; padding: 10px; border-radius: 4px; font-family: monospace; color: #4CAF50; word-break: break-all; }
    ...
    .warning { background: #ff6b35; color: white; padding: 10px; border-radius: 4px; margin: 10px 0; }
    .info { background: #4CAF50; color: white; padding: 10px; border-radius: 4px; margin: 10px 0; }
  </style>
</head>
<body>
  <div class="container">
    <h1>ESP32-CAM Internet Controller</h1>
    
    <div class="access-info">
      <h3>🌐 Internet Access Information</h3>
      <div><strong>Local Access:</strong> http://192.168.1.253</div>
      <div><strong>Public IP:</strong> <span id="public-ip">Checking...</span></div>
      <div><strong>External Access URL:</strong></div>
      <div class="access-url" id="external-url">Checking public IP...</div>
      <div class="info" style="margin-top: 10px; font-size: 12px;">
        📋 <strong>Router Setup Required:</strong> Forward port 8080 to 192.168.1.253:80 in your router settings
      </div>
    </div>
    ...
    <div class="stream-container">
      ...
      <div id="stream-placeholder" style="color: #666; padding: 50px;">
        ...
        <small>Optimized for 10+ FPS | Accessible from anywhere!</small>
      </div>
    </div>
  </div>
      ...
      fetch('/stats')
        ...
        .then(data => {
          ...
          if (data.public_ip) {
            ...
            document.getElementById('external-url').textContent = 'http://' + data.public_ip + ':8080';
          }
...
void setup() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (currentPublicIP != "") {
      ...
      Serial.printf("   Forward external port %d to 192.168.1.253:80\n", EXTERNAL_PORT);
      Serial.println("   Enable port forwarding in your router's settings\n");
    }
  } else {
...
void setupWebServer() {
  // Serve main page (with optional authentication)
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request){
    if (!authenticateUser(request)) {
      return request->requestAuthentication("ESP32-CAM", "Please enter credentials");
    }
    request->send_P(200, "text/html", index_html);
  });
  
  // Stats endpoint (with public IP info)
  server.on("/stats", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    json += "\"public_ip\":\"" + currentPublicIP + "\"";
    json += "}";
